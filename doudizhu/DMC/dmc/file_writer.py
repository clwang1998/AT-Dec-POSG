...
class FileWriter:
    def __init__(self,
        ...
        if xp_args is None:
            xp_args = {}
        # Keep a lightweight but complete metadata trail so each run can be
        # reproduced from disk without relying on external tracking tools.
        self.metadata = {
            'date_start': datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'),
            'date_end': None,
            'successful': False,
            'xpid': self.xpid,
            'args': copy.deepcopy(xp_args),
            'platform': {
    ...
    def close(self, successful: bool = True) -> None:
        self.metadata['date_end'] = datetime.datetime.now().strftime(
            '%Y-%m-%d %H:%M:%S.%f')
        self.metadata['successful'] = bool(successful)
        self._save_metadata()

    def _save_metadata(self) -> None:
        with open(self.paths['meta'], 'w') as jsonfile:
            json.dump(self.metadata, jsonfile, indent=2, sort_keys=True)
